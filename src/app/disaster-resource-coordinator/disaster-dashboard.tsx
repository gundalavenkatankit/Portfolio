...
export function DisasterDashboard({ useShelterPage = true }: { useShelterPage?: boolean }) {
  ...
  return <>
    ...
    <header className="reliefHeader"><Link href="/disaster-resource-coordinator" className="reliefBrand"><span>RR</span><strong>ReliefReady</strong></Link><nav aria-label="Primary navigation">{useShelterPage && <Link href="/disaster-resource-coordinator/my-area">My area</Link>}<a href="#alerts">Alerts</a>{useShelterPage && <Link href="/disaster-resource-coordinator/shelters">Shelters</Link>}{useShelterPage && <Link href="/disaster-resource-coordinator?assistant=open">Ask ReliefReady</Link>}<a href="#resources">Resources</a><a href="#prepare">Prepare</a></nav><Link href="/">Portfolio</Link></header>
    <main id="reliefMain">
      ...
      <section className="resourceSection" id="resources"><div className="reliefSectionIntro"><div><p className="reliefEyebrow">Trusted assistance</p><h2>Find the right resource</h2></div><p>These tools use official or nationally recognized emergency resources.</p></div><div className="resourceGrid">{useShelterPage && <><Link href="/disaster-resource-coordinator?assistant=open"><span>01 · Assistant</span><h3>Ask ReliefReady</h3><p>Ask questions across alerts, declarations, shelter records, recovery centers, and locations.</p><strong>Start a conversation →</strong></Link><Link href="/disaster-resource-coordinator/my-area"><span>02 · Coordinator</span><h3>My area summary</h3><p>Enter one location to coordinate alerts, shelters, declarations, and recovery support.</p><strong>Check my area →</strong></Link><Link href="/disaster-resource-coordinator/shelters"><span>03 · Shelter</span><h3>Shelter Finder</h3><p>Search FEMA shelter records by state, city, postal code, or shelter name.</p><strong>Find shelters →</strong></Link><Link href="/disaster-resource-coordinator/declarations"><span>04 · Federal status</span><h3>Disaster declarations</h3><p>Check recent federal declarations, affected areas, and declared assistance programs.</p><strong>Check declarations →</strong></Link><Link href="/disaster-resource-coordinator/recovery-centers"><span>05 · Recovery</span><h3>Recovery center finder</h3><p>Find active FEMA centers for application help, case questions, and recovery guidance.</p><strong>Find recovery centers →</strong></Link></>}{(useShelterPage ? resources : [previousShelterResource, ...resources]).map((resource, index) => <a href={resource.url} target="_blank" rel="noreferrer" key={resource.name}><span>0{index + (useShelterPage ? 6 : 1)} · {resource.type}</span><h3>{resource.name}</h3><p>{resource.detail}</p><strong>Open resource →</strong></a>)}</div></section>
